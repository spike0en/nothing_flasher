...
##----------------------------------------------------------##
if [ ! -d "$(pwd)/platform-tools-r35.0.0" ]; then
    if [[ $OSTYPE == 'darwin'* ]]; then
        fastboot_dl="https://dl.google.com/android/repository/platform-tools_r35.0.0-darwin.zip"
    else
        fastboot_dl="https://dl.google.com/android/repository/platform-tools_r35.0.0-linux.zip"
    fi
    curl -L "$fastboot_dl" -o "$(pwd)/platform-tools-r35.0.0.zip"
    unzip "$(pwd)/platform-tools-r35.0.0.zip"
    rm "$(pwd)/platform-tools-r35.0.0.zip"

    if [ -d "$(pwd)/platform-tools" ]; then
        mv "$(pwd)/platform-tools" "$(pwd)/platform-tools-r35.0.0"
    fi
fi

fastboot="$(pwd)/platform-tools-r35.0.0/fastboot"
