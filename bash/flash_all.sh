...
##----------------------------------------------------------##
if [ ! -d "$(pwd)/platform-tools-r37.0.1" ]; then
    if [[ $OSTYPE == 'darwin'* ]]; then
        fastboot_dl="https://dl.google.com/android/repository/platform-tools_r37.0.1-darwin.zip"
    else
        fastboot_dl="https://dl.google.com/android/repository/platform-tools_r37.0.1-linux.zip"
    fi
    curl -L "$fastboot_dl" -o "$(pwd)/platform-tools-r37.0.1.zip"
    unzip "$(pwd)/platform-tools-r37.0.1.zip"
    rm "$(pwd)/platform-tools-r37.0.1.zip"

    if [ -d "$(pwd)/platform-tools" ]; then
        mv "$(pwd)/platform-tools" "$(pwd)/platform-tools-r37.0.1"
    fi
fi

fastboot="$(pwd)/platform-tools-r37.0.1/fastboot"
